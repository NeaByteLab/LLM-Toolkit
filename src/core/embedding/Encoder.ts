...
import type { EmbeddingOptions, EmbeddingResult, EmbeddingExtractor } from '@interfaces/Embedding'
...
export class Encoder implements EmbeddingExtractor {
  ...
  private async processBatch(
    ...
    return Array.from(outputData.data).map((_: unknown, index: number) => {
      const start: number = index * (outputData.dims[1] ?? 0)
      const end: number = start + (outputData.dims[1] ?? 0)
      return Array.from(outputData.data.slice(start, end))
    })
  }
